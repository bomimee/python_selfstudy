...
for destination in sheet_data:
    ...
    try:
        ...
        print(f"⚠️ Error fetching flights for {destination['city']}: {e}")
        continue

    cheapest_flight = find_cheapest_flight(flights)
    print(f"{destination['city']}: {cheapest_flight.price}")

    data_manager.update_destination_price(destination["id"], cheapest_flight)
    time.sleep(2)
...
# ==================== Send Notifications and Emails  ====================
    ...
    if cheapest_flight.price != "N/A" and cheapest_flight.price < destination["lowestPrice"]:
        ...
        if cheapest_flight.stops == 0:
            message = f"Low price alert! Only {cheapest_flight.price} Won to fly direct \n from {cheapest_flight.origin_airport} to {cheapest_flight.destination_airport} \n on {cheapest_flight.out_date} until {cheapest_flight.return_date}"
        else:
            message = f"Low price alert! Only {cheapest_flight.price} Won to fly \n from {cheapest_flight.origin_airport} to {cheapest_flight.destination_airport} \n with {cheapest_flight.stops} stops \n on {cheapest_flight.out_date} until {cheapest_flight.return_date}"
